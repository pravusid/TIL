...
코틀린 표준 라이브러리는 인자의 개수에 따라 `Function0<R>`, `Function1<P1, R>` 등의 인터페이스를 제공한다.
...
자바8 이전의 자바에서는 필요한 `FunctionN` 인터페이스의 `invoke` 메소드를 구현하는 익명 클래스를 넘기면 된다.
...
만약 인라인 함수를 호출하면서 람다대신 함수 타입의 변수를 넘기면 인라인 함수를 호출하는 코드위치에서는 변수에 저장된 람다의 코드를 알 수 없다.
