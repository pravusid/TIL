...
SQL 쿼리에서 GROUP BY를 사용해서, 행의 그룹에 집계함수를 적용하고 그룹당 한 행만 얻는 작업을 한다.
그러나 GROUP BY 쿼리는 에러가 발생하거나 결과를 신뢰할수 없는 경우가 발생하기도 한다.

### 안티패턴: 그룹되지 않은 컬럼 참조

- 단일 값 규칙
  - 각 그룹의 행은 GROUP BY 절 뒤에 쓴 컬럼(목록)의 값이 같은 행이다
  - 쿼리에서 SELECT 목록에 있는 모든 컬럼은 그룹당 하나의 값을 가져야 한다(Single-Value Rule)
  - 따라서, GROUP BY 뒤에 나오는 컬럼(목록) 또는 집계 함수의 인수로 사용되는 컬럼 이외의 다른 컬럼은 불명확한 값으로 취급된다

- 내 뜻대로 동작하는 쿼리
  - 쿼리에 집계함수를 실행했을 때 다른 컬럼의 값은 집계함수에서 출력된 값과 동일행임을 보장할 수 없다
  - 출력되는 값은 Single-Value Rule을 따라야 한다

### 안티패턴 인식 방법: 그룹되지 않은 컬럼 참조

대부분의 DBMS에서는 단일 값 규칙을 위반하는 쿼리에 대해 에러를 출력한다

SQLite, MySQL에서는 모호한 컬럼에 신뢰할 수 없는 값이 들어갈 수도 있다.
MySQL에서는 그룹의 (물리적 스토리지의)첫 번째 행의 값이 반환된다.
SQLite에서는 그룹의 마지막 행의 값을 반환한다.
이는 문서화 된 것이 아니며, 향후 버전에서 변경될 가능성이 조금이라도 존재한다.

### 안티패턴 사용이 합당한 경우: 그룹되지 않은 컬럼 참조

MySQL, SQLite는 단일 값 규칙에 맞지 않아도 신뢰할 수 없는 결과를 출력한다.

원칙적으로 단일 값 규칙을 위배하는 컬럼 값이, 출력되는 다른 컬럼과 명확히 1:1 대응이 되는 경우가 있을 수 있다.
이런 종류의 명확한 관계를 functional dependency라고 부른다.

functional dependency의 일반적인 예는 테이블의 PK와 테이블 속성간의 관계이다

### 해법: 컬럼을 모호하게 사용하지 않기

- functional dependency 컬럼만 쿼리
  - 모호한 컬럼은 쿼리에서 제거한다

- 상호 연관된 서브쿼리(correlated subquery) 사용
  - 상호 연관 서브쿼리는 바깥쪽 쿼리에 대한 참조를 가지고 있어 바깥쪽 쿼리의 각 행에 대해 다른 결과를 생성할 수 있다
  - 상호 연관 서브쿼리는 바깥쪽 쿼리의 한 행에 대해 한번씩 실행되므로 성능상 최적 방법이 아니다
  - `SELECT ~ FROM X a WHERE NOT EXIST (SELECT * FROM X b WHERE <식별자> AND <조건>)`

- 유도 테이블(derived table) 사용
  - 서브쿼리를 유도 테이블로 사용해 임시 결과를 만들고, 테이블과 조인해 원하는 쿼리 결과를 얻는다
  - 유도 테이블을 사용하면 상호 연관 서브쿼리보다 확장성이 좋다
  - `SELECT ~ FROM X a JOIN (SELECT <식별자>, <집계(조건)> FROM X GROUP BY <식별자>) b ON <식별자> AND <조건> GROUP BY <식별자>`

- 조인 사용
  - outer join을 사용하면 대응되는 행이 없는곳에서는 이 행에 대한 모든 컬럼에 NULL이 채워진 결과가 출력된다
  - 성능이 중요한 경우 join을 사용하는 것이 가장 좋다
  - `SELECT ~ FROM X a LEFT OUTER JOIN Y b ON <식별자> AND (a.조건 < b.조건 AND a.식별자 < b.식별자) WHERE b.식별자 IS NULL`

- 다른 컬럼에 집계 함수 사용
  - 다른 컬럼에 집계함수를 사용해도 명확한 결과가 출력될 때만 사용한다

- 각 그룹에 대해 모든 값을 연결
  - MySQL, SQLite는 그룹에 속한 모든 값을 하나의 값으로 연결하는 `GROUP_CONCAT()` 함수를 지원한다
  - 함수는 기본값으로 쉼표로 구분된 문자열을 생성한다 (해당 그룹의 컬럼 값 전체를 연결해서 출력)
  - 전체가 출력될 뿐 추가 정보는 없다 (어떤 값이 최신인지...) 또한 표준 SQL이 아니다
...
쿼리에서 임의 행을 고르는 가장 흔한 기법은 데이터를 임의로 정렬한 다음 첫 행을 고르는 것이다.
`SELECT * FROM Foobar ORDER BY RAND() LIMIT 1;`

`RAND()`와 같은 비결정적 수식으로 정렬하면 인덱스를 활용할 수 없게 된다.
이는 쿼리 성능을 고려할 때 문제가 된다.
또한 전체 데이터 집합을 정렬하는데 비용이 많이 드는데 작업 결과의 대부분은 낭비가 된다(하나만 사용하므로).

### 해법: 테이블 전체 정렬 피하기

임의 정렬은 테이블 스캔과 비용이 큰 수동정렬을 동반한다

- 1과 MAX 사이에서 임의의 키 값 고르기
  - PK 값이 1부터 시작해서 연속적으로 존재할 때
  - 1과 PK의 최댓값 사이에서 임의의 키값을 고를 수 있다

- 다음으로 큰 키 값 고르기
  - 1과 PK의 최댓값 사이에서 고른 임의의 키 값보다 크거나 같은 값중 첫번째 값을 사용한다
  - 키 값이 균일하게 분포하지 않을 수 있기 때문에 모든 키 값이 동일한 빈도로 선택되는 것이 중요하지 않을 때 사용한다

- 모든 키 값의 목록을 구한 다음, 임의로 하나 선택
  - 결과 집합의 PK 값 하나를 선택하는 애플리케이션 코드를 사용할 수 있다
  - 데이터베이스로부터 모든 값을 불러올 때 자원을 과다하게 사용할 수 있다
  - 쿼리를 두번 해야 한다 (전체 목록 요청, 임의의 값을 선택하고 그 값을 요청)
  - 결과 집합의 크기가 적당하고 불연속 목록에서 값을 선택할 때 사용할 수 있다

- OFFSET을 이용해 임의 선택
  - 데이터집합에서 행의 개수를 세고 0과 행 개수 사이에서 임의의 수를 고른다음
  - 데이터 집합을 쿼리 할 때 이 수를 오프셋으로 사용한다
  - 이 방법은 표준이 아닌 `LIMIT` 절에 의존한다 (MySQL, PostgreSQL, SQLite 에서 사용가능)
  - 다른 방법은 Oracle, SQL Server, DB2에서 동작하는 `ROW_NUMBER()` 함수를 사용하는 것이다

- 벤더 종속적 방법
  - DBMS마다 이런 종류의 작업을 처리하는 방법이 있을 수 있다
  - SQL Server(TABLESAMPLE), Oracle(SAMPLE) ...

## 가난한 자의 검색 엔진

어느 애플리케이션이든 단어나 구절로 텍스트를 검색할 필요가 생긴다.

관계형 데이터베이스 기본 원리 중 하나는 컬럼의 값이 원자적이어야 한다.
즉, 한 값을 다른 값과 비교할 수 있지만 비교할 때는 항상 전체 값이어야 한다는 것이다.

SQL에서 부분 문자열을 비교하는 것은 비효율적이거나 부정확하다.

### 안티패턴: 패턴 매칭 사용

SQL 문자열 비교를 위해 패턴 매칭 기능을 제공하며, 이 중 널리 지원 되는 것은 `LIKE` 연산자이다.
표준적인 방법은 아니지만 정규 표현식 또한 많은 데이터베이스 제품에서 지원된다.

패턴 매칭 연산자의 가장 중요한 단점은 성능이 나쁘다는 것이다.
일반 인덱스를 활용할 수 없기 때문에 테이블의 모든 행을 스캔해야 한다.
두 번째 문제는 LIKE나 정규표현식을 사용한 단순한 패턴 매칭이 원치 않은 다른 결과도 찾을 수 있다는 것이다.
...
SQL 프로그래머들이 일하면서 가장 흔하게 빠지는 문제가 "이걸 어떻게 하나의 쿼리로 할 수 있을까?"라는 생각을 하는 것이다.

프로그래머들은 SQL 쿼리가 어렵고, 복잡하고, 비용이 많이 든다고 배웠기 때문에, SQL 쿼리가 두개면 두배로 나쁘다고 생각한다.

### 안티패턴: 복잡한 문제를 한 번에 풀기

- 의도하지 않은 제품
  - 모든 결과를 하나의 쿼리로 만들어내려고 할 때 나타나는 흔한 결과 중 하나가 카테시안 곱(Cartesian product)이다
  - 카테시안 곱은 쿼리에 사용된 두 테이블에 이들의 관계를 제한하는 조건이 없을 때 발생한다

- 충분하지 않다면
  - 한 번에 처리하려는 쿼리는 잘못된 결과를 얻을 뿐만 아니라 작성하기도 어렵고 수정/디버깅도 어렵다
  - 많은 조인과 연관된 서브쿼리, 다른 연산을 사용해야 하는 복잡한 SQL 쿼리는 엔진이 최적화하여 빠르게 실행하기도 어렵다
  - SQL 쿼리가 같은 복잡도를 가진다면 적은 수의 쿼리를 실행하는 것이 맞지만, 복잡도는 실행비용을 지수적으로 증가시킬 수도 있다

### 해법: 분할 정복

Ockham's razor: 두 이론이 동일한 예측을 한다면, 단순한 쪽이 좋은 이론이다

- 한 번에 하나씩
  - 의도하지 않은 카테시안 곱이 생기는 두 테이블 사이에 논리적 조인 조건을 찾을 수 없다면 조건이 아예 없기 때문일 수 있다
  - 카테시안 곱을 피하려면 복잡한 쿼리를 단순한 여러개의 쿼리로 나누어야 한다
  - SQL 엔진은 단순한 쿼리를 쉽계 최적화하고 실행할 수 있다

- UNION 연산
  - UNION 연산을 사용하면 여러 쿼리의 결과를 하나의 결과 집합으로 묶을 수 있다
  - 하나의 쿼리를 실행하여 하나의 결과 집합을 받는것이 정말 필요하다면 UNION이 유용할 수 있다
  - 서브쿼리의 컬럼이 호환될 때만 UNION 연산을 사용할 수 있다

## 암묵적 컬럼

프로그래머들이 타이핑을 너무 많이 해야 한다고 불평하는 예 중 하나가 사용할 모든 컬럼을 SQL 쿼리에 써야 하는 경우이다
따라서, SQL 와일드카드를 사용하게 되는데, 이렇게 사용된 컬럼 목록은 암묵적이다.

### 안티패턴: 지름길만 좋아하면 길을 잃는다

컬럼 이름 지정 없이 와일드카드를 사용하면 타이핑을 줄일 수는 있지만, 몇 가지 위험을 초래할 수 있다

- 리팩토링 방해
  - 암묵적 컬럼을 사용하는 INSERT 문에서는 테이블의 모든 컬럼에 대한 값을 테이블에 정의된 순서대로 지정해야 한다
  - SELECT 쿼리를 실행한 결과 값에 접근할 때 컬럼 순서를 사용한다면, 컬럼 순서가 변경되었을 경우 다른 값이 된다
  - 이를 애플리케이션 출력 부분에서 문제를 인지한다면 어느 부분에서 오류가 발생했는지 추적하기 어려워 진다

- 숨겨진 비용
  - 쿼리에서 와일드 카드를 사용하는 편리함은 성능과 확장적응성에 부정적일 수 있다
  - 실제로 사용하는 데이터는 일부인데 항상 전체를 전송할 수 있다

- 요청한 것을 얻는다
  - SQL에 원하지 않는 컬럼을 제외한 모든 컬럼을 지정하는 문법은 없다
  - 테이블에 모든 컬럼을 요청하는 와일드카드를 사용하거나 원하는 컬럼 목록을 명시적으로 나열해야 한다

### 안티패턴 사용이 합당한 경우: 지름길만 좋아하면 길을 잃는다

조인 쿼리에서는 각 테이블에 개별적으로 와일드카드를 사용할 수 있다.
와일드카드 앞에 테이블 이름이나 별칭을 붙이면 된다.

때로는 실행 효율 보다 개발 효율을 중요하게 여길 때가 있다.
이 때도 짧고 읽기 쉬운 쿼리를 작성하기 위해 와일드카드를 사용할 수 있다.
...
- 오류 검증
  - 컬럼을 명시적으로 지정하면 다음 문제에 대한 저항력이 생긴다
  - 테이블에 컬럼 위치가 바뀌어도 쿼리 결과는 동일하다
  - 테이블에 컬럼이 추가되어도 쿼리 결과에 나타나지 않는다
  - 테이블에 컬럼이 삭제되면 쿼리가 에러를 발생시킨다 (빠르게 오류를 잡을수 있게 한다 - fail fast)

- 네트워크 대역폭을 효율적으로 사용한다

- 항상 와일드카드를 쓸 수는 없다
  - 특정 컬럼에 수식을 적용하거나 별칭을 적용하기 위해 컬럼을 명시해야 한다
  - 개별적으로 컬럼을 다룰 변경사항이 발생한다면 와일드카드에서 변경하는 것이 보다 고통스러울 것이다
...
- 패스워드 저장
  ...
  - 데이터베이스 서버에서 SQL 쿼리 로그를 검색할 수 있다
  - 서버나 백업 미디어에 저장되어 있는 데이터베이스 백업 파일로부터 데이터를 읽을 수 있다

- 패스워드 인증
  - 사용자가 로그인을 시도하면 애플리케이션은 사용자의 입력과 데이터베이스에 저장된 인증정보 문자열을 비교한다
  - 이 경우 평문은 다양한 방법으로 노출될 수 있고, 인증 요청이 발생할 때마다 위험이 존재한다
  - 두 개의 다른 조건을 한 덩어리로 만들어서도 안된다
    - WHERE 절에서 `user_name = ? AND password = ?` 같은 조건은 인증 실패 원인을 알 수 없다
    - 또한 상대적으로 SQL injection 등에 취약하다

### 안티패턴 사용이 합당한 경우: 패스워드를 평문으로 저장

- 애플리케이션에서 패스워드를 이용해 다른 서드파티 서비스에 접근해야 할 수 있다
- 이런 경우에는 패스워드를 읽을 수 있는 형식으로 젖아해야 한다
- 그러나 데이터베이스에 평문으로 저장하기보다는 애플리케이션에서 풀 수 있는 암호화 기법을 사용하는 것이 좋다

### 해법: 패스워드의 소금 친 해시 값을 저장한다

- SQL에서 해시 사용
  - SHA256으로 문자열을 변환하면 길이는 항상 64이다
  - 해시함수는 SQL 표준에 포함되어 있지 않다

- 해시에 소금 추가
  - 패스워드 대신 해시값을 저장했으로 공격자가 데이터베이스에 접근했다면 결국 비밀번호를 알아낼 수도 있다
  - 이 것을 어렵게 하는 방법 중하나는 패스워드를 부호화 할 때 소금(salt)를 추가하는 것이다
  - salt는 해시 값을 구하기 전에 원본값에 덧붙이는 무의미한 바이트열이다

- SQL에서 패스워드 숨기기
  - SQL 쿼리에 패스워드를 평문으로 넣지 않고 애플리케이션 코드에서 해시값을 계산해 SQL 쿼리에서 해시값을 사용하여야 한다
  - 이 경우 네트워크 패킷이나 로그에 평문 패스워드가 노출되는 것을 방지할 수 있다

- 패스워드 복구 대신 재설정
  - 해시값을 원복할 수는 없다
  - 패스워드를 잊어버렸으면 패스워드를 변경하도록 강제해야 한다

## SQL Injection

SQL은 애플리케이션 코드와 함께 사용될 수 있다.
쿼리 문자열과 애플리케이션 변수를 엮어 SQL 문자열로 만드는 것을 보통 Dynamic SQL이라고 한다.

이 경우 어려운 부분은 개발자가 의도하지 않은 동작은 허용하지 않도록 하는 것이다.

### 안티패턴: 검증되지 않은 입력을 코드로 실행

SQL injection의 고전적인 예는 SQL 삽입되는 문자열에 세미콜론을 넣어 문장을 끝내고 두 번째 문장을 실행하는 것이다.

`SELECT FROM Bugs WHERE bug_id = 1; DELETE FROM Bugs`

- 사고는 발생할 것이다
  - 데이터에 quote가 포함되어 있을 수 있다
  - DBMS에서는 이를 문법오류로 받아들이겠지만, 유사한 오류가 여러곳에서 발생할 수 있다

- 최고의 웹 보안 위협
  - 요청 파라미터가 SQL문에서 어떻게 사용되는지를 추측할 수 있는 공격자는 파라미터에 의도적인 문자열을 추가할 수 있다
  - SQL injection은 파싱되기 전의 SQL문을 조작하는 방법으로 동작한다
  - SQL문이 파싱되기 전에 동적인 부분을 삽입하는 한, SQL injection 위험이 있는 것이다

#### SQL Injection 위험 대비

- 값을 이스케이프
  - quote와 같은 특수 문자를 escape 처리한다
  - 대부분의 데이터베이스 제품도 역슬래시를 이용한 이스케이프를 지원한다
  - 이 기법은 문자열이 아닌 경우에는 효과적이지 않을 수 있다

- 쿼리 파라미터
  - SQL injection에 대한 대비책으로 가장 자주 인용되는 것은 쿼리 파라미터를 사용하는 것이다
  - SQL 문자열에 동적 값을 삽입하는 대신 쿼리를 만들 때 파라미터가 들어갈 자리를 미리 정하는 것이다
  - DBMS는 쿼리를 준비(prepare)할 때 SQL을 파싱한다, 이후에는 SQL 쿼리에 아무런 변경을 가할 수 없다
  - 사실 쿼리 파라미터를 사용하는 것은 SQL injection에 대한 강력한 방어이기는 하지만 보편적 해결이 될 수는 없다
  - 쿼리 파라미터의 값은 하나의 리터럴 값으로 해석되기 때문이다
    - 값의 목록을 파라미터로 전달할 수 없다: `IN (?)`
    - 테이블 이름은 파라미터로 전달할 수 없다: `FROM ?`
    - 컬럼 이름을 파라미터로 전달할 수 없다
    - SQL 키워드를 파라미터로 전달할 수 없다

- Stored Procedure
  - 저장 프로시저는 고정된 SQL문을 포함하며, 프로시저를 정의할 때 파싱된다
  - 그러나 저장 프로시저 안에서도 안전하지 않은 방법으로 동적 SQL을 사용할 수 있다 (`QUOTE()` 함수등을 통해...)

- 데이터 접근 프레임 워크
  - 데이터 접근 프레임워크에서 SQL 문을 문자열로 작성하는 것을 허용한다면 잠재적 위험이 존재한다
  - 어떠한 프레임워크도 안전한 SQL 코드만 작성하도록 강제할 수 없다

### 안티패턴 인식 방법: 검증되지 않은 입력을 코드로 실행

실질적으로 모든 데이터베이스 애플리케이션은 SQL 문을 동적으로 생성한다.
SQL문의 어느 부분이든 문자열을 연결하거나 값을 문자열에 삽입해 만든다면, 잠재적인 SQL injection 공격 위험에 노출되는 것이다.

### 해법: 아무도 믿지 마라

- 입력 값 필터링
  - 입력에 대해 유효하지 않은 문자는 모두 제거해야 한다
  - 처리 방식은 프로그래밍 언어에 따라 다르다

- 파라미터를 통한 값 전달
  - 쿼리의 동적인 부분이 단순한 값이면 쿼리 파라미터를 사용해 다른 SQL 표현과 분리해야 한다

- 동적 값 인용하기
  - 보통 쿼리 파라미터를 사용하는 것은 최상의 방법이지만, 간혹 쿼리 파라미터 사용으로 optimizer가 잘못된 인덱스를 사용할 수 있다.
  - 이 경우 SQL문에 직접 값을 삽입할 수 있지만, 문자열을 주의 깊게 인용해야 한다
  - 앞에서 본 IN 연산자의 경우 실행시점에 파라미터의 개수를 알 수 있다
  - 이 경우 IN 연산자 다음 파라미터 위치(`?`)를 동적으로 생성한 PreparedStatement를 사용할 수 있다

- 사용자의 입력을 코드와 격리
  - 쿼리 파라미터와 이스케이핑은 리터럴 값을 SQL문과 묶을 수 있으나 테이블 이름이나 컬럼이름, SQL 키워드 등을 사용할 수는 없다
  - 이 경우 동적 생성된 쿼리에 사용자 요청을 결합하기 전 사용자 요청을 검증해야 한다
  - 문자열이나 숫자에 대해 모든 값을 선택하게 하는 것은 데이터 값에 대한 것이지 식별자나 키워드에 대한 것이 아니기 때문이다
  - 선택가능한 사용자 선택을 사전정의하여 생성해두고 사용자가 제한된 선택 이내의 값을 전송했는지 확인한다
  - 사전 정의된 값중 사용자의 선택을 꺼내서 SQL문과 결합한다

## 가상키 편집증

수열 중간에 숫자가 몇 개 빠지면 불편함을 느끼는 사람들이 있다.
...
- 시퀀스에서 벗어난 번호 할당
  ...
  - 그러나 사용되지 않은 가장 작은 값을 찾으려면 불필요한 셀프조인을 실행해야 한다 (조건: `a.id + 1 = b.id`)
  - 또한 사용할 수 있는 최대값을 조회한 뒤 삽입에 사용하는 것은 동시성 문제가 있음을 안다

- 기존 행의 번호를 다시 매기기
  - 기존 행에 대한 키 값을 업데이트 해 틈을 메우고 모든 값을 연속이 되도록 하는 경우가 있다
  - PK 값이 가장 큰 행을 찾아 사용하지 않는 가장 작은 키 값으로 업데이트 하는 경우가 있을 것이다
  - 앞의 경우와 비슷한 방법으로 사용되지 않는 키값을 찾아야 한다
  - 마찬가지로 재할당 하기 위해 UPDATE 문을 사용하는데, 이는 동시성 문제가 존재한다
  - 또한 번호를 새로 붙인 행을 참조하는 모든 자식 행에 변경된 키 값을 전파해야 한다
    - ON UPDATE CASCADE 옵션으로 외래키를 선언했다면 상대적으로 작업이 쉽겠지만 누락이 없으리라는 보장은 없다
    - 그렇지 않다면 수작업으로 제약조건을 비활성화 한다음 업데이트하고 제약조건을 복원해야 한다
  - 이런 과정을 거쳤더라도 결과는 일시적이다. 다시 새로운 가상키 값에 비어있는 틈이 발생한다

- 데이터 불일치 만들기
  - PK 값을 재사용하는 것은 좋은 생각이 아니다
  - 납득할만한 이유로 행을 삭제했거나, 롤백 결과로 틈이 발생한 것일 수 있기 때문이다

### 해법: 극복하라

PK 값은 유일하고 NULL이 아니어서 각 행을 참조하는 데 사용할 수 있어야 한다.
행을 식별하는 데 숫자가 연속적이어야 할 필요는 없다.

- 행에 번호 매기기
  - 대부분의 가상키 생성기는 행 번호처럼 보이는 번호를 반환한다
  - 이들은 단조 증가이지만 이는 유일성을 보장하는 가장 편한 방법이기 때문이다
  - 행 번호를 PK와 혼동하면 안된다. PK는 한 테이블에서 한 행을 식별하지만, 행 번호는 결과 집합에서 행을 식별하는 것이기 때문이다
  - 행 번호를 사용하면 좋은 때는 쿼리 결과의 부분만 반환하는 경우이다. 이를 종종 pagination이라 부른다
  - 이를 위해서는 단조증가하는 행번호를 사용해야 하는데, `ROW_NUMBER()`와 같은 함수들이 존재한다
  - MySQL, SQLite 등에서는 LIMIT, SKIP을 지원한다

- GUID 사용하기
  - 어떤 수를 한 번 이상 사용하지 않으면서 랜덤한 가상키 값을 생성할 수 있다
  - 이런 목적으로 Global Unique Identifier를 지원한다
  - GUID는 128비트 의사난수로 보통 32자리 16진수로 표현된다
  - GUID를 사용하면 중복을 걱정하지 않고 여러 데이터베이스 서버에서 동시에 가상키를 생성할 수 있다
  - 그러나, 값이 길고 입력하고 저장하기에 나쁘며 처리속도가 느리다. 또한, 랜덤 값이므로 패턴을 추론할 수 없다.

- 정수 소진 공포증
  - 단조 증가 가상키는 언젠가 소진될 것이란 걱정이 있다
  - 32비트 정수는 최대 2^32개의 구분된 값을 표현한다
  - 1초에 하나씩 하루 24시간 행을 삽입한다면 32비트 정수형의 가상키를 소진하는데 136년이 걸린다
  - 64비트 정수형을 사용한다면 584,542년이 걸린다

- 자연키 사용
  - PK 값을 받아들이기에는 너무나도 민감하다면 변경이 발생하지 않는 식별 가능한 자연키를 사용할 수도 있다

## 나쁜 것 안 보기

프로그래머에게는 가능한 코드를 제거하려는 본능적 우선순위가 있다

### 안티패턴: 짚 없이 벽돌 만들기

개발자는 다음과 같은 두 가지 형태로 나쁜 것 안보기 안티패턴을 사용한다
하나는 데이터베이스 API 반환값을 무시하는 것이고, 다른 하나는 애플리케이션 코드에 산재해있는 SQL 코드의 단편만을 보는 것이다.

- 진단 없는 진료
  - 데이터베이스를 연결하고 처리하는 함수로부터 반환되는 값을 무시하는 경우가 있다
  - 반환 값을 무시한다면 어디에서 문제가 발생했는지 알 수 없다

- 행간 읽기
  - SQL 쿼리를 문자열로 생성하는 애플리케이션 코드만 계속 쳐다볼 수 있다
  - 이런식으로 디버깅하는 것은 그림을 보지않고 그림 맞추기 퍼즐을 푸는 것과 같다
...
전문적인 프로그래머들은 프로젝트에서 좋은 소프트웨어 공학적 best practice를 따르려 노력한다.

### 안티패턴: SQL을 2등 시민으로 만들기

애플리케이션 코드를 개발할 때 관례를 따르는 개발자조차도 데이터베이스 코드를 작성할 때는 이런 관례로 부터 면제된다고 생각하는 경향이 있다.

### 해법: 초당적 품질 문화 확립

소프트웨어 공학의 전체 생명주기에는 다음 세 부분이 포함되는데 QA도 수반된다

1. 프로젝트 요구사항을 문서로 명확하게 명세한다
2. 요구사항에 대한 솔루션을 설계하고 개발한다
3. 솔루션 요구사항이 맞는지 검토하고 테스트한다

#### 스키마 도구

RoR은 소스 관리하에 데이터베이서 업그레이드를 관리하는 마이그레이션(migration)이라는 기법을 보급했다.

데이터베이스를 한 단계 업그레이드 하는 스크립트를 Ruby에 기반해 작성한다.
업그레이드 함수의 변경을 반대로 하는 다운그레이드 스크립트도 함께 작성한다.

마이그레이션 도구는 마이그레이션을 자동으로 실행하고 테이블을 생성해 현재 데이터베이스에 적용한 리비전을 기록한다.

## 마법의 콩

MVC 모델에서 모델이 단지 접근 객체일 뿐이라 가정하는 지나친 단순화가 존재할 수 있다

### 안티패턴: 액티브 레코드인 모델

간단한 애플리케이션에서 모델에 맞춤 로직이 많이 필요하지 않다.
모델 객체의 필드를 데이터베이스 테이블 하나의 컬럼과 매치하는 것은 상대적으로 쉽고 단순하다.

마틴 파울러는 이런 매핑을 지원하는 디자인 패턴을 Active Record라 불렀다.

액티브 레코드는 데이터 접근 패턴이다.
데이터베이스의 테이블이나 뷰에 대응하는 클래스를 정의하고, CRUD에 대응하는 메소드를 호출한다.

RoR은 액티브 레코드를 보급했고, 이 패턴은 웹 개발 프레임워크에서 Data Access Object의 사실상 표준이 되었다.

액티브 레코드를 사용하는 것은 아무런 잘못이 없다.
하나의 테이블에 있는 개별 행에 대한 단순한 인터페이스를 제공하는 좋은 패턴이다.

안티패턴은 MVC 애플리케이션에서 모든 모델 클래스가 액티브 레코드 클래스를 상속하는 관행이다.

#### 누설되기 쉬운 추상화

조엘 스폴스키는 추상화를 사용해서 생산적이 되려고 할 때, 내부 구조를 알아야 한다면 누설되기 쉬운 추상화라 표현했다.
MVC에서 액티브 레코드를 모델로 사용하는 것은 누설되기 쉬운 추상화의 예이다.

액티브 레코드에서 간단한 작업을 할 경우 마법처럼 동작하지만,
모든 형태의 데이터베이스 접근에 사용하려 할 경우 SQL에서는 간단한 것이 복잡하게 표현된다는 것을 알게된다.

#### 액티브 레코드와 안티패턴

- 액티브 레코드는 모델을 스키마와 결합시킨다
  - 액티브 레코드 클래스는 데이터베이스의 테이블 하나 또는 뷰 하나를 나타낸다
  - 액티브 레코드 객체의 각 필드는 대응 되는 테이블의 각 컬럼과 매치된다
  - 이는 새로운 구조의 데이터를 표현하기 위해 데이터베이스를 리팩토링 할 때 모델클래스도 수정하고 애플리케이션 코드도 수정해야 함을 뜻한다

- 액티브 레코드는 CRUD 함수를 노출시킨다
  - 모델 클래스를 사용하는 사람이 의도한 사용방법을 우회해 직접 CRUD 함수로 데이터를 업데이트 할 수 있다

- 액티브 레코드는 빈약한 도메인 모델을 조장한다
  - 모델이 기본적인 CRUD 메소드 외에는 아무런 동작도 가지지 않는 경우가 많다
  - 모델을 단순한 데이터 접근 객체로 취급하면 비즈니스 로직은 모델 외부에 존재하게 되고 이로인해 모델 동작의 응집도가 낮아진다
  - 마틴 파울러는 이러한 안티패턴을 빈약한 도메인 모델(Anemic Domain Model)이라 불렀다

- 마법의 콩은 단위 테스트가 어렵다
  - 마법의 콩 안티패턴을 사용하면 MVC 각 계층을 테스트하기 어려워진다
  - 모델
    - 액티브 레코드와 같은 클래스로 만들었기 때문에, 데이터 접근과 분리해 모델의 동작을 테스트할 수 없다
    - 모델을 테스트하려면 실제로 데이터베이스 쿼리를 실행해야 한다
  - 컨트롤러
    - 데이터 접근 객체인 모델로 인하여 여러 컨트롤러에 동일한 코드가 반복해서 나타나는데 이를 모두 테스트해야 한다

### 안티패턴 인식 방법: 액티브 레코드인 모델

다음 단서가 안티패턴의 징후이다

- 모델에 맞춤 SQL 쿼리를 어떻게 넘길까?
  - 이는 모델 클래스를 데이터베이스 접근 클래스로 사용하는 것이다
  - 모델에 SQL 쿼리를 넘길 필요가 없어야 한다
  - 모델 클래스는 필요한 쿼리가 어떤 것이든 캡슐화 해야 한다

- 복잡한 모델 쿼리를 어떻게 하면 반복하지 않을까?
  - 모델안에 쿼리를 코딩하고 모델의 인터페이스로 노출해야 한다

- 모델을 단위 테스트 하기위해 데이터베이스 픽스처를 작성해야 할까?
  - 데이터베이스 픽스처는 데이터베이스 접근을 테스트하는 것이지 비즈니스 로직을 테스트하는 것이 아니다
  - 데이터베이스와 격리된 상태에서 모델을 단위테스트 할 수 있어야 한다

### 안티패턴 사용이 합당한 경우: 액티브 레코드인 모델

- 테이블 개별행에 대해 간단한 연산만 하는 단순 데이터 접근객체만 필요한 경우
- 빠르게 작성하고 동작하는 프로토타입을 작성하는 경우

### 해법: 액티브 레코드를 가지는 모델

- 정보 전문가
  - 어떤 동작에 책임이 있는 객체는 동작을 수행하는데 필요한 모든 데이터를 가지고 있어야 한다
  - 액티브 레코드는 한 번에 한 테이블과 동작하므로 여러 개의 데이터 접근 객체를 모아 합성 동작에 사용할 다른 클래스가 필요하다
  - 모델과 액티브 레코드와 같은 DAO 사이의 관계는 IS-A가 아닌 HAS-A여야 한다
  - 모델이 DAO 클래스를 상속하는 것이 아니라 DAO를 사용한다면 모델링이 해야 하는 도메인을 위한 모든 데이터와 코드를 포함하도록 설계할 수 있다

- 창조자
  - 모델이 데이터를 데이터베이스에 저장하는 방법은 내부적 구현 상세여야 한다
  - 애플리케이션의 컨트롤러와 뷰는 모델이 데이터를 가져오고 저장하는 방법을 알 필요없이, 도메인 모델의 인터페이스만을 사용해야 한다

- 낮은 결합도
  - 논리적으로 독립적인 코드 블록을 분리해야 한다
  - 애플리케이션에 대한 요구사항이 있으므로 복잡도가 코드 어느곳에는 존재해야 하는데, 최적의 위치를 잘 판단해야 한다

- 높은 응집도
  - 도메인 모델 클래스의 인터페이스는 의도된 사용법을 반영해야지 데이터베이스의 물리적 구조나 CRUD 동작을 반영하면 안된다
  - 액티브 레코드의 인터페이스인 `find()`, `save()` 등으로는 애플리케이션의 어떤 요구사항에 적용되는지 제대로 알기 어렵다
  - `assignUser()`와 같은 메소드가 명시적이고 이해하기도 쉽다

Eric Evans는 도메인 모델이라는 더 좋은 방식을 설명한다.

모델은 애플리케이션을 위한 비즈니스 로직을 구현하는 곳이다.
데이터를 데이터베이스에 저장하는 것은 모델의 내부적 구현 상세이다.

모델을 데이터베이스 레이아웃이 아닌 애플리케이션 개념에 따라 설계하면 데이터베이스 동작을 모델 클래스 속에 완전히 숨길 수 있다.

또한 모델을 DAO에서 분리하면 DAO의 stub과 mock을 생성하여 모델의 단위테스트를 도울 수 있다.

## 정규화 규칙

정규화(normalization) 규칙은 복잡하지 않지만 미묘하다.

### 관계형의 뜻

관계형(relational)이란 용어는 테이블 사이의 관계를 뜻하는게 아니라, 테이블 자체 또는 테이블과 컬럼 사이의 관계를 뜻한다.
관계란 규칙과, 규칙을 따르는 쌍의 부분집합이란 두 가지 뜻으로 사용된다.

테이블 정규화를 시작하기 전에, 이들이 적절한 관계인지 확인해야 한다.

- 행은 위아래 순서를 가지지 않는다
  - SQL에서 ORDER BY 절을 사용해 순서를 지정하지 않는 한, 쿼리는 예상할 수 없는 순서의 결과를 반환한다

- 컬럼은 좌우 순서를 가지지 않는다

- 중복 행은 허용되지 않는다
  - 중복을 방지하기 위해 한 행을 다른 행과 구분하는 방법이 있어야 하고 개별행에 접근할 수 있어야 한다
  - SQL에서는 이를 보장하기 위해 유일하게 행을 식별하도록 PK 제약조건을 선언한다
  - 모든 컬럼은 하나의 타입만 갖고, 각 행당 하나의 값만 갖는다
...
- 정규화를 적용한 후에 다른 테이블 속성을 가져오기 위해 조인이 필요하다
- 반정규화는 한 형태의 쿼리에 대해서는 편해지고 성능에도 도움이되지만, 다른 쿼리에 대해서는 많은 비용을 초래한다
- 반정규화 사용이 합당한 경우도 있지만, 데이터베이스를 정규화된 형태로 만드는 것이 우선이다
- 정규화에서 데이터를 자식으로 분리하고 가상키를 반드시 사용해야 하는 것은 아니다
- 비즈니스 데이터베이스의 20% 이상이 제 3정규형 까지는 만족하지만, 제 4정규형에는 위배된다고 한다

### 정규화란

정규화의 목표는 다음과 같다

- 우리가 이해할 수 있는 방법으로 실세계에 대한 사실을 표현한다
- 사실을 중복해 저장하는 것을 줄이고 데이터에 이상(anomaly)이 생기거나 모순이 생기는 것을 방지한다
- 정합성을 지원한다

정규화는 데이터를 올바르게 저장하고 문제가 생기는 것을 회피하는 것이지 성능을 높이는 것은 아니다.

테이블이 정규화 규칙을 만족하면, 그 테이블을 정규형(normal form)이라고 한다.
정규화의 각 단계를 나타내는 전통적인 정규형은 다섯 개가 있다.
각 정규형은 관계를 설계함에 있어 특정 형태의 중복이나 이상을 제거한다.

보통 테이블이 한 정규형을 만족하면, 이전의 정규형을 모두 만족한다.
