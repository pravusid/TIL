...
전역코드, `eval()` 함수로 실행되는 코드, 함수안의 코드를 실행할 경우이다.
...
전역 실행 컨텍스는 일반 실행 컨텍스트와는 달리, arguments 객체가 없으며 전역 객체 하나만을 포함하는 스코프 체인이 있다.
...
초기화는 변수나 함수에 해당하는 표현식이 실행되기 전까지는 이루어지지 않는다.(초기화 이전까지 undefined가 할당됨)
