...
분산 시스템은 통신 네트워크를 통하여 서로 약하게 결합된 프로세서들의 집합으로, 이 프로세서들은 메모리와 클럭을 공유하지 않는다.

분산 시스템 내의 하나의 프로세서 관점에서 그 프로세서가 가지고 있는 자원을 로컬이라고 하고,
그 프로세서 외의 나머지 모든 자원을 원격이라고 한다.

일반적으로 한 사이트 호스트의 자원을 다른 사이트 호스트가 사용한다고 할 때
자원을 가진 쪽을 서버라 하고, 자원을 이용하는 쪽을 클라이언트라고 한다.

##### 분산 시스템의 목적

- 자원 공유: 다른 사이트의 자원을 이용할 수 있다
- 연산속도 향상: 특정 연산이 동시에 처리될 수 있는 여러개로 분할된다면 여러 사이트에서 분산 처리 할 수 있다
- 신뢰성 향상: 한 사이트에서 결함이 발생해도 나머지 사이트들은 계속 동작할 수 있다
- 통신 용이성: 네트워크를 통해 연결되어 있을 때 사용자들 간 정보교환이 가능하다
...
- LAN: 근거리 통신망이므로 비교적 협소한 지역에 적합하게 설계
- WAN: 광범위한 사용자들이 연결되는 네트워크

#### 분산 운영체제 자원

- 데이터 이주: 원격지의 데이터를 필요한 곳으로 전송하여 사용함
- 계산 이주: 데이터가 이주하는 대신 원격 프로시저 호출(RPC)을 통해 원격지에서 처리한 후 결과를 받아온다
- 프로세스 이주: 프로세스 자체를 원격지로 이주하여 부하분산, 계산속도 향상이 가능하다

### 분산 파일시스템

분산 파일 시스템(Distributed File System)은 클라이언트가 서버에 저장된 파일을 마치 로컬 파일인 것처럼 처리할 수 있는 클라이언트/서버 기반 파일 시스템이다.

원격 파일에 대한 접근 요청은 2가지 상호 보완 방법으로 처리된다.
파일 접근 요청은 서버로 전달되고, 서버는 요청을 처리하고 결과를 클라이언트들에게 반환한다.

요청한 데이터가 캐시에 없으면 데이터 복사본을 서버로부터 클라이언트의 캐시로 가져오고 이후 접근 요청은 캐시된 복사본에서 실행된다.
캐시된 복사본을 마스터 파일과 일치시키는 문제를 캐시 일관성 문제라고 한다.

### 원격 프로시저 호출

#### 원격 프로시저 호출 동작

RPC는 전통적인 프로시저 호출을 중간애 가로채 해당 호출과 매개변수를 원격지 컴퓨터로 전달한다.

RPC를 사용하려는 클라이언트는 일단 로컬 프로시저를 호출해야 하는데, 이러한 프로시저를 stub 루틴이라고 부른다.
...
스텁 루틴의 수행이 재개되고 RPC 결과 메시지를 풀어서 이를 다시 호출자에게 돌려준다.

호출 프로세스의 관점에서 보면 이러한 구조는 마치 한 컴퓨터에서 다른 컴퓨터로의 프로시저 호출처럼 동작한다.

#### 원격 프로시저 호출 구현

- 이상적 RPC 구문은 고급 프로그래밍 언어에서 로컬 프로시저 호출과 같다
- 호출 및 피호출 프로시저가 각각의 주소공간 안에서 실행되기 때문에 대부분의 RPC 구현은 참조호출을 지원하지 않는다
